...
public:
	...
	size_t pad(unsigned len)
	{
		...
		// pad with pseudo-random bytes. Zero-padding is a problem for small files:
		// the fountain-coded blocks derived from them are zero-heavy, which produces
		// large uniform regions in the encoded image -- which in turn generates false
		// anchor detections that displace the real corner anchors on the receive side.
		// skippable-frame content is ignored by the decompressor, so any bytes are safe.
		std::string temp(len-8, '\0');
		uint32_t s = 0x6d2b79f5 ^ static_cast<uint32_t>(len);
		for (char& c : temp)
		{
			s = s*1664525u + 1013904223u;
			c = static_cast<char>((s >> 16) & 0xFF);
		}
